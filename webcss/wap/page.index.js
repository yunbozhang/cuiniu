var pageIndex = new function() {
	...
	this.shopmore = function(type) {
		...
		kj.ajax.post(url,dataval , function(data){
			...
			pageIndex.shoppages = kj.toint(obj.value);
			if(pageIndex.shoppage >= pageIndex.shoppages ) {
				kj.hide("#id_init_refresh");
			} else {
				kj.show("#id_init_refresh");
			}
			pageIndex.showstate();
			if(pageIndex.shoppages == 0) {
				objul.innerHTML += '<li class="xnone">没有搜索的商家</li>';
			}
			pageIndex.ispagein = false;
		});
	}
   this.getLocation = function(){
	   ...
	   if(navigator.geolocation){
			...
			navigator.geolocation.getCurrentPosition(function(position) {
				//返回用户位置
				//经度
				pageIndex.longitude =position.coords.longitude;
				//纬度
				pageIndex.latitude = position.coords.latitude;
				pageIndex.shopmore();
			},function(error) {
				pageIndex.shopmore();
			   switch(error.code){
	...
	this.showsort = function() {
		if(kj.obj("#id_act_sort").style.display!='none') {
			kj.hide("#id_act_sort");
			return;
		}
	...
	this.showtype = function() {
		if(kj.obj("#id_typelist").style.display!='none') {
			kj.hide("#id_typelist");return;
		}
	...
	this.showcondition = function() {
		if(kj.obj("#id_condition").style.display!='none') {
			kj.hide("#id_condition");
			return;
		}
	...
	this.showstate = function() {
		...
		if(pageIndex.shoppages>pageIndex.shoppage && ii>=pageIndex.pagesize*pageIndex.shoppage) {
			kj.show("#id_listmore");
		} else {
			kj.hide("#id_listmore");
		}
