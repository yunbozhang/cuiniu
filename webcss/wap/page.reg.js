var pageReg = new function() {
	...
	this.reg_email = function() {
		...
		kj.ajax.get(kj.cfg('baseurl') + '/common.php?app=sys&app_act=verify_reg&type=' + type + '&key=' + key + '&verifycode=' + verifycode, function(data) {
			...
			if(obj_data.isnull) {
				...
				if(obj_data.code == 0) {
					pageReg.show_verify_email();
					pageReg.start_time(1);
				} else {
					...
					if(obj_data.code == '11') {
						...
						if(pageReg.reg_verify) kj.hide('#winverifycodereg');
					}
				}
			}
		});
	...
	this.start_time = function(isstart) {
		...
		if(timer<0) {
			...
			obj.innerHTML = timer;
			setTimeout('pageReg.start_time()' , 1000);
		}
	}
