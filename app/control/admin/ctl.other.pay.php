...
class ctl_other_pay extends mod_other_pay {
	...
	function act_record() {
		...
		//相关属性
		$this->arr_type =  cls_obj::get('cls_com')->pay("get_config" , "type" );
		$this->arr_state =  cls_obj::get('cls_com')->pay("get_config" , "state" );
		$this->arr_method = cls_config::get("" , "pay" , "" , "");
		return $this->get_view();
	}
	function act_refund() {
		$id = (int)fun_get::get("id");
		$arr_return = cls_obj::get('cls_com')->pay("on_refund",$id);
		return fun_format::json($arr_return);
	}
}