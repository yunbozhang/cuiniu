...
class mod_article extends inc_mod_common {
	 ...
	 */
	function article_view( $id , $key = '') {
		$obj_db = cls_obj::db();
		$where = ' where article_isdel=0 and article_state>0 ';
		if(empty($key)) {
			$where .= "and article_id='" . $id . "'";
		} else {
			$where .= "and article_key='" . $key . "'";
		}
		$obj_article = $obj_db->get_one("select * from " . cls_config::DB_PRE . "article" . $where);
		...
		if(!empty($obj_article["article_folder_id"])) {
			...
			if(!empty($obj_folder)) $obj_article = array_merge($obj_article , $obj_folder);
		...
		$obj_article['addtime'] = date("Y-m-d" , $obj_article['article_addtime']);
		$obj_article['article_content'] = fun_get::filter($obj_article['article_content'] , true);
		...
		//act 也可能存在点号
		unset($arr_act[0]);
		unset($arr_act[count($arr_act)]);
		$act = implode("." , $arr_act);
		$view_dir = fun_is::wap() ? 'wap' : cls_config::get_env('view_dir');
		$viewpath = cls_resolve::get_config("view_root") . "/" . $view_dir . "/" . fun_get::basename($tpl);
		if(!is_file($viewpath)) {
			$view_dir = fun_get::basename(dirname($tpl));
		}
		$arr_folder = array(
			"dir" => $view_dir,
			"app" => $arr[0],
		...
		$str_cont = cls_app::on_display($arr_folder["dir"] , '' , $arr_folder["app"] , $arr_folder["act"], array("article"=>$obj_article));
