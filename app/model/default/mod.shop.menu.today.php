 ...
 * 2016-08-30
 */
class mod_shop_menu_today extends inc_mod_shop {
	...
	function sql_list( $today_date , $today_date_period) {
		...
		$str_where .= " and today_shop_id='" . $this->shop_info["id"] . "'";
		$obj_db = cls_obj::db();
		//取字段信息
	...
	function menu_list() {
		...
		$obj_result = $obj_db->select("SELECT today_menu_id FROM ".cls_config::DB_PRE."meal_menu_today where today_date='" . $get_today_date . "' and today_date_period='" . $get_today_date_period . "' and today_shop_id='" . $this->shop_info["id"] . "'");
		while( $obj_rs = $obj_db->fetch_array($obj_result) ) {
			$arr_id[] = $obj_rs["today_menu_id"];
		}
		$str_where = " where menu_mode=2";//只允许选择自定义模式的菜品
		if(count($arr_id) > 0 ) $str_where .= " and menu_id not in(" . implode(",", $arr_id) . ")";
		$str_where .= " and menu_shop_id='" . $this->shop_info["id"] . "'";
	...
	function on_save() {
		...
		for( $i = 1 ; $i < $lng_count ; $i++) {
			$arr_fields = array(
				...
				"today_shop_id" => $this->shop_info["id"],
			);
