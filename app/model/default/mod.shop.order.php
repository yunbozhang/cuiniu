...
class mod_shop_order extends inc_mod_shop {
	...
	function get_pagelist() {
		...
		//合并查询数组
		if(!cls_obj::get("cls_user")->is_admin()) $arr_where[] = "order_shop_id='" . $this->shop_info["id"] . "'";
	...
	function get_today_list($where = '') {
		...
		if(!cls_obj::get("cls_user")->is_admin()) {
			$str_where .= " and order_shop_id='" . cls_obj::get("cls_user")->shop_id . "'";
		}
		if( !empty($where) ) $str_where .= $where;
		$arr_state = tab_meal_order::get_perms('state');
		$obj_result = $obj_db->select("select a.*,b.shop_name,b.shop_tel,b.shop_pic from " . cls_config::DB_PRE . "meal_order a left join " . cls_config::DB_PRE . "meal_shop b on a.order_shop_id=b.shop_id" . $str_where . " order by order_id desc");
		while($obj_rs = $obj_db->fetch_array($obj_result)) {
	...
	function get_newnum() {
		...
		if(!cls_obj::get("cls_user")->is_admin()) $str_where .= " and order_shop_id='" .$this->shop_info["id"] . "'";
	...
	function on_accept() {
		...
		$obj_db = cls_obj::db_w();
		$where = (cls_obj::get("cls_user")->is_admin()) ? "" : "order_shop_id='" . $this->shop_info["id"] . "'";
	...
	function on_refuse() {
		...
		$where = 'order_state=0';
		if(!cls_obj::get("cls_user")->is_admin()) $where .= " and order_shop_id='" . $this->shop_info["id"] . "'";
		$arr = tab_meal_order::on_state($id , -1 , $beta , $where);
	...
	function on_state() {
		...
		$where = (cls_obj::get("cls_user")->is_admin()) ? "" : " and order_shop_id='" . $this->shop_info["id"] . "'";
		$arr = tab_meal_order::on_state($str_id , $state , $beta , "order_state=0" . $where);
		if($arr['code'] != 0) return $arr;
		return $arr_return;
	}
	/* 确认定单并奖励
	...
	function on_award() {
		...
		if(!empty($arr_id)) $str_id = $arr_id; //优先考虑 arr_id
		$where = (cls_obj::get("cls_user")->is_admin()) ? "" : "order_shop_id='" . $this->shop_info["id"] . "'";
		$arr = tab_meal_order::on_award($str_id,$where);
		if($arr['code'] != 0) {
			$arr_return['code'] = $arr['code'];
			$arr_return['msg']  = $arr['msg'];
		}
	...
	function on_delete() {
		...
		if(!empty($arr_id)) $str_id = $arr_id; //优先考虑 arr_id
		$where = (cls_obj::get("cls_user")->is_admin()) ? "" : "order_shop_id='" . $this->shop_info["id"] . "'";
		$arr = tab_meal_order::on_delete($str_id,$where);
		if($arr['code'] != 0) {
			$arr_return['code'] = $arr['code'];
			$arr_return['msg']  = $arr['msg'];
		}
