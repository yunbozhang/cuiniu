 ...
 * 2016-08-30
 */
class mod_shop_act extends inc_mod_shop {
	/* 按模块查询菜单信息并返回数组列表
	 * module : 指定查询模块
	 */
	function get_pagelist() {
		$arr_where = array("act_isdel=0 and act_shop_id='" . cls_obj::get('cls_user')->shop_id . "'");
		$arr_where_s = array();
		$str_where = '';
		$lng_issearch = 0;
		//取查询参数
	...
	function on_delete() {
		...
		$arr = tab_meal_act::on_delete($str_id , "act_shop_id='" . $this->shop_info['id'] . "'");
		if($arr['code'] != 0) {
			$arr_return['code'] = $arr['code'];
			$arr_return['msg']  = $arr['msg'];
		}
