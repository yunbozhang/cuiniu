...
class mod_shop_report extends inc_mod_shop {
	...
	function order_num_byyear() {
		...
		$obj_result = $obj_db->select("SELECT  left(order_day,7) as 'tips'," . $field . " as 'val' FROM " . cls_config::DB_PRE . "meal_order where order_shop_id='" . $this->shop_info["id"] . "' and left(order_day,4)='" . $date . "' group by left(order_day,7)");
	...
	function order_num_bymonth() {
		...
		$field = "count(1)";
		if($channel == "money") $field = "sum(order_total_pay)";
		$obj_result = $obj_db->select("SELECT  order_day as 'tips'," . $field . " as 'val' FROM " . cls_config::DB_PRE . "meal_order where order_shop_id='" . $this->shop_info["id"] . "' and left(order_day,7)='" . $date . "' group by order_day");
		while($obj_rs = $obj_db->fetch_array($obj_result)) {
			$tips = substr($obj_rs["tips"] , -2);
			$arr_list[$tips] = $obj_rs["val"];
		}
	...
	function order_num_byday() {
		...
		$field = "count(1)";
		if($channel == "money") $field = "sum(order_total_pay)";
		$obj_result = $obj_db->select("SELECT  left(order_time,13) as 'tips'," . $field . " as 'val' FROM " . cls_config::DB_PRE . "meal_order where order_shop_id='" . $this->shop_info["id"] . "' and order_day='" . $date . "' group by left(order_time,13)");
		while($obj_rs = $obj_db->fetch_array($obj_result)) {
			$tips = substr($obj_rs["tips"] , -2);
			$arr_list[$tips] = $obj_rs["val"];
		}
