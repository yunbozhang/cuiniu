...
class mod_ajax extends inc_mod_default {
	...
	function save_order(){
		...
		$str_ids = $arr[1];
		$shop_id = $arr_cart['shop_ids'][0];

		//查看店铺运营信息
		$obj_shop = $obj_db->get_one("select shop_state,shop_user_id,shop_sms,shop_sms_tel,shop_extend,shop_support_score from " . cls_config::DB_PRE . "meal_shop where shop_id='" . $shop_id . "'");
		if(empty($obj_shop)) {
			return array("code"=>500 , "msg"=>"您选择的餐馆不存在");
		}
		if($obj_shop["shop_state"]<1) {
			return array("code"=>500 , "msg"=>"您选择的餐馆暂时未开通网上订餐服务，请选择其它餐馆");
		}
		...
		if(!empty($obj_shop['shop_support_score'])) $arr_order['order_isaward'] = -1;
		...
		while($obj_rs = $obj_db->fetch_array($obj_result)) {
			if($obj_rs["menu_state"]<1 || $obj_rs["menu_shop_id"]!=$shop_id) {
				$arr_return['code']=500;
				$arr_return['msg']="选择菜品【" . $obj_rs["menu_title"] . "】不存在,请重新选择后再提交";
				return $arr_return;
			}
			//按星期
			if($obj_rs['menu_mode'] == 1 && !stristr(",".$weekday."," , "," . $obj_rs["menu_weekday"] . ",")) {
				$arr_return['code']=500;
				$arr_return['msg']="选择菜品【" . $obj_rs["menu_title"] . "】今天暂不提供,请重新选择后再提交";
				return $arr_return;
			}
			//按日期
			...
			if($obj_rs["menu_mode"] == 2) {
				...
				if($obj_rs["num"]>0 && $obj_rs["num"] <substr_count("," . $menu_ids . "," , $obj_rs["menu_id"])) {
					$arr_return["code"]=1;
					$arr_return["msg"]="【" . $obj_rs["menu_title"]."】当天份量不足，请选择其它菜品";
					return $arr_return;
				}
			}
		...
		if(count($arr_today_menu)>0) {
			...
			while($obj_rs = $obj_db->fetch_array($obj_result)) {
				...
				if($num>0 && $num <substr_count("," . $menu_ids . "," , $obj_rs["today_menu_id"])) {
					$arr_return["code"]=1;
					$arr_return["msg"]="【".$arr_menu["id_" . $obj_rs["today_menu_id"]]["menu_title"]."】当天份量不足，请选择其它菜品";
					return $arr_return;
				}
		...
		if($lng_score_money>0) {
			...
			if(empty($score_mode) || ($score_mode==2 && empty($obj_shop['shop_support_score']))) {
				$arr_return["code"]=500;
		...
		if(!empty($shop_act_id) && is_array($shop_act_id)) {
			...
			$arr_act_where = $this->get_shop_act($arr_order['order_shop_id'] , $arr_order['order_total'] , $num);
		...
		foreach($arr as $item){
			...
			if(in_array($item , $arr_today_menu)) {
				...
				$obj_db->on_exe("update ".cls_config::DB_PRE."meal_menu_today set today_sold=today_sold+'".$num."' where today_menu_id='".$item."' and today_date='".$today_date."' and today_date_period='".$today_date_period."'" . $where_today);
			}else {
				$num = $arr_menu['id_'.$item]['menu_num'] - $arr_menu['id_'.$item]['num'];
				$obj_db->on_exe("update ".cls_config::DB_PRE."meal_menu set menu_sold_today='".$num."',menu_sold_time='".TIME."' where menu_id='".$item."'");
			}
		}
		//清除购物车信息
		cls_session::set_cookie("cart_ids","");
		//发送短信提醒
		if($obj_shop["shop_sms"] == 1 && !empty($obj_shop["shop_sms_tel"]) ) {//订单提醒短信
			$arr_tel = explode("," , $obj_shop["shop_sms_tel"]);
			$tel = $arr_tel[rand(0,count($arr_tel)-1)];//随机一个电话
			$arr = cls_obj::get('cls_com')->sms("on_send" , array("tel"=>$tel , "cont" => cls_config::get("neworder_shop_tips","meal") ,"id" => $arr_return["id"] , "type"=>1) );
		} else if($obj_shop["shop_sms"] == 2) {//订单详细短信
			$arr = $arr_cart["shop_" . $shop_id]["cart"];
			$ii = 1;
			$arr_i = array();
			foreach($arr as $item) {
				$arr_x = array();
			...
			$cont = "新订单：" . implode("；" , $arr_j) . " 合计：" . $arr_order["order_total"] . "；";
			if(!empty($score_money)) $cont .= "抵扣：" . $score_money . "；应收：" . $arr_order["order_total_pay"];
	...
	function on_save_info() {
		...
		$arr_area_id = array();
		if($this->area_id!=0) $arr_area_id[] = $this->area_id;
		if($this->area_lou_id!=0) $arr_area_id[] = $this->area_lou_id;
		$arr_area_id[] = fun_get::post("area_allid");
		$arr_info['info_area_allid'] = implode("," , $arr_area_id);
		$arr = $this->save_info($arr_info);
		if($arr["code"] != 0){
			$arr_return['code']=101;
			$arr_return['msg']=$arr['msg'];
	...
	function save_info($arr_info) {
		...
		if(empty($arr_info["info_name"])){
			$arr_return["code"]=1;
			$arr_return["msg"]="请输入您的称谓";
			return $arr_return;
		}
			
		if(empty($arr_info["info_louhao1"])){
			$arr_return["code"]=1;
			$arr_return["msg"]="请填写您的楼层号";
			return $arr_return;
		}
		if(empty($arr_info["info_tel"]) && empty($arr_info["info_mobile"])){
			$arr_return["code"]=1;
			$arr_return["msg"]="请输入您的联系电话";
			return $arr_return;
		}
		$arr_area_id = array();
		if($this->area_id!=0) $arr_area_id[] = $this->area_id;
		if($this->area_lou_id!=0) $arr_area_id[] = $this->area_lou_id;
		$arr_area_id[] = $arr_info['info_area_allid'];
		$arr_info['info_area_allid'] = implode("," , $arr_area_id);
	...
	function on_reg($user_type = ''){
		...
		if(cls_config::get('rule_uname','user')=='email') {
			$arr = tab_sys_verify::on_verify($arr_user['user_name'] ,$verifycode , 3 ,86400);
			if($arr['code'] != 0) return $arr;
			cls_obj::get("cls_session")->set('verify_reg' , 1);//设置已验证标识
		} else if(cls_config::get('rule_uname','user')=='mobile') {
			$arr = tab_sys_verify::on_verify($arr_user['user_name'] ,$verifycode , 4 , 600);
			if($arr['code'] != 0) return $arr;
			cls_obj::get("cls_session")->set('verify_reg' , 1);//设置已验证标识
	...
	function on_findpwd_step2() {
		...
		if($method == 1) {
			...
			$url = cls_config::get("url" , 'base') . "/index.php?app_act=findpwd.email&val=" . $obj_user['user_email'] . "&key=" . $arr_key['key'];
			//取邮件内容
	...
	function on_verify_mobile() {
		...
		$arr_user = cls_obj::get("cls_user")->get_user($uname , false);
		if(empty($arr_user) || !isset($arr_user[$uname])) return array('code' => '500' , 'msg' => '验证账号不存在');
		$arr = tab_sys_verify::on_verify($arr_user[$uname] , $key , 2);
		if($arr['code'] == 0) {
