...
class mod_other_email extends inc_mod_admin {
	...
	function on_save() {
		...
		if(!empty($arr_fields['email_id']) && $arr_fields["email_serverinfo"]["password"] === '*******************************') {
			$obj_rs = cls_obj::db()->edit(cls_config::DB_PRE."other_email" , "email_id='" . $arr_fields['email_id'] . "'");
			if(!empty($obj_rs["email_serverinfo"])) {
				$obj_rs["email_serverinfo"] = unserialize($obj_rs["email_serverinfo"]);
				$arr_fields["email_serverinfo"]["password"] == $obj_rs["email_serverinfo"]["password"];
			}
		}
