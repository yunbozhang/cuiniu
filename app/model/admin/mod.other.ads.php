...
class mod_other_ads extends inc_mod_admin {
	...
	function get_editinfo($msg_id) {
		...
		//店铺名
		$obj_rs["shop_name"] = '';
		if(!empty($obj_rs["ads_shop_id"])) {
			$obj_rs2 = cls_obj::db()->get_one("select shop_id,shop_name from " . cls_config::DB_PRE . "meal_shop where shop_id='" .$obj_rs["ads_shop_id"] . "'");
			if(!empty($obj_rs2)) {
	...
	function on_save() {
		...
		if(fun_is::set("ads_shop_id")) {
			$arr_fields['ads_shop_id'] = (int)fun_get::get("ads_shop_id");
			if($arr_fields['ads_shop_id']>0) $arr_fields['ads_position'] .= "-shop" . $arr_fields['ads_shop_id'];
		}
