...
class mod_meal_menu_group extends inc_mod_meal {
	...
	function on_save_all() {
		...
		$arr_group_mode = fun_get::get("group_mode");
		...
		$str_where = "group_shop_id='" . $shop_id . "'";
		if( !empty($str_ids) ) {
			$str_where .= " and not group_id in(".$str_ids.")";
		}
		//首先删除没在保存id中的所有记录
		tab_meal_menu_group::on_delete(array(),$str_where);
		for( $i = 1 ; $i < $lng_count ; $i++) {
			$arr_fields = array(
				"group_id" => (int)$arr_group_id[$i],
				"group_name" => $arr_group_name[$i],
				"group_sort" => $arr_group_sort[$i],
				"group_mode" => $arr_group_mode[$i],
				"group_shop_id" => $shop_id
			);
