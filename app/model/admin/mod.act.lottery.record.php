 ...
 * 2016-08-30
 */
class mod_act_lottery_record extends inc_mod_admin {
	/* 按模块查询菜单信息并返回数组列表
	 * module : 指定查询模块
	 */
	function get_pagelist() {
		...
		if( !empty( $arr_search_key['s_wxid'] ) ) $arr_where_s[] = "record_wx_id='" . $arr_search_key['s_wxid'] . "'"; 
		if( !empty( $arr_search_key['s_lotteryid'] ) ) $arr_where_s[] = "record_lottery_id='" . $arr_search_key['s_lotteryid'] . "'"; 
		if( !empty( $arr_search_key['s_awardid'] ) ) $arr_where_s[] = "record_award_id='" . $arr_search_key['s_awardid'] . "'"; 
		//合并查询数组
	...
	function sql_list($str_where = "" , $lng_page = 1) {
		...
		$obj_db = cls_obj::db();
		//取字段信息
		$arr_cfg_fields = tab_sys_user_config::get_fields("act.lottery.record" , $this->app_dir , "act");
		$arr_return['tabtd'] = $arr_cfg_fields["tabtd"];
		$arr_return['tabtit'] = $arr_cfg_fields["tabtit"];
		//取排序字段
		$arr_config_info = tab_sys_user_config::get_info("act.lottery.record"  , $this->app_dir);
		...
		$arr_return["pageinfo"] = $obj_db->get_pageinfo(cls_config::DB_PRE."act_lottery_record" , $str_where , $lng_page , $lng_pagesize);
		$obj_result = $obj_db->select("SELECT " . $arr_cfg_fields["sel"] . " FROM ".cls_config::DB_PRE."act_lottery_record a left join " . cls_config::DB_PRE . "act_lottery b on a.record_lottery_id=b.lottery_id left join " . cls_config::DB_PRE . "act_lottery_award c on a.record_award_id=c.award_id left join " . cls_config::DB_PRE . "sys_user d on a.record_user_id=d.user_id" . $str_where . $sort . $arr_return['pageinfo']['limit']);
