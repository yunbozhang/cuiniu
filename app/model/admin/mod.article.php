...
class mod_article extends inc_mod_admin {
	...
	function on_save_article() {
		...
		if(fun_is::set('article_folder_id')) {
			$arr_fields['article_folder_id'] = fun_get::get("article_folder_id");
		}
	...
	function on_save_folder() {
		...
		$arr_fields = array(
			...
			"folder_key" => fun_get::post("folder_key"),
		);
	...
	function get_user_type_list($channel_id , $type) {
		...
		$arr_return["pageinfo"] = $obj_db->get_pageinfo("(SELECT count(1) as num FROM kj_article a inner join " . cls_config::DB_PRE . "meal_shop b on a.article_about_id=b.shop_id where article_channel_id='" . $channel_id . "' group by article_about_id) tab1" , '' , (int)$page , $lng_pagesize);
		if($type == 'shop') {
