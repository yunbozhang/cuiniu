 ...
 * 2016-08-30
 */
class mod_meal_dispatch extends inc_mod_meal {
	...
	function area_list($get_url_pid) {
		...
		$arr_id = array();
		$arr_pids = array();
		$obj_result = $obj_db->select("SELECT dispatch_area_id,area_pids FROM " . cls_config::DB_PRE . "meal_dispatch a left join " . cls_config::DB_PRE . "sys_area b on a.dispatch_area_id=b.area_id where dispatch_shop_id='" . $this->admin_shop["id"] . "'");
		while( $obj_rs = $obj_db->fetch_array($obj_result) ) {
	...
	function on_delete() {
		...
		$where = '';
		if(!empty($pid)) {
			$where = "dispatch_area_id in(select area_id from " . cls_config::DB_PRE . "sys_area where " . $obj_db->concat(',','area_pids',',') . " like '%," . $pid . ",%') and dispatch_shop_id='" . $shop_id . "'";
		}
	...
	function get_area_path($pid , $topid=0) {
		$arr_return = array();
		...
		if($obj_rs = cls_obj::db()->fetch_array($obj_result))	{
			$arr_return[] = $obj_rs;
			$arr = $this->get_area_path($obj_rs['area_pid'] , $topid);
			if(count($arr)>0) {
