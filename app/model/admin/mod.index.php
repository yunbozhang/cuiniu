...
class mod_index extends inc_mod_admin {
	...
	function get_count_info() {
		...
		//今日 回头用户
		$obj_rs = $obj_db->get_one("select count(1) as num from " . cls_config::DB_PRE . "sys_user where user_regtime<='" . $today_time . "' and user_logintime>'" . date("y-m-d" , $today_time) . "'");
	...
	function order_num() {
		...
		if(!empty($limit_area)) {
			...
			$where .= " and order_shop_id in(select shop_id from " . cls_config::DB_PRE . "meal_shop where " . implode(" or " , $arr_x) . ")";
		}
