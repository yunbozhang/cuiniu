	...
	*返回的验证结果：true/false
    ...
    function notify_verify() {
		...
		if(empty($_POST)) {							//判断POST来的数组是否为空
			...
			$arr = array('app','app_act','paymethod');
			$arr_get = $_POST;
			foreach($arr as $item) {
				if(isset($arr_get[$item])) unset($arr_get[$item]);
			}

			$post          = para_filter($arr_get);	//对所有POST返回的参数去空
	...
	*return 验证结果：true/false
    ...
    function return_verify() {
		...
		if(empty($_GET)) {							//判断GET来的数组是否为空
			...
			$get          = para_filter($arr_get);	    //对所有GET反馈回来的数据去空
	...
	*return 服务器ATN结果集
    ...
    function get_verify($url,$time_out = "60") {
		...
		if(function_exists('stream_socket_client')) {
			$fp = stream_socket_client($transports . $urlarr['host'] . ":" . $urlarr['port'],$errno,$errstr,$time_out);   //  Timeout (seconds)
		} else {
			$fsockopen = (function_exists("fsockopen")) ? "fsockopen" : "pfsockopen";
