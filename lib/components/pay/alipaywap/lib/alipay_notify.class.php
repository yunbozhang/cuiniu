...
class AlipayNotify {
	...
	function verifyReturn(){
		if(empty($_GET)) {//判断GET来的数组是否为空
			...
			$arr_get = isset($_GET['klkkdj_oldgeturl']) ? $_GET['klkkdj_oldgeturl'] : $_GET;
			unset($arr_get['app']);
			unset($arr_get['app_act']);
			unset($arr_get['paymethod']);
			//生成签名结果
			$isSign = $this->getSignVeryfy($arr_get, $arr_get["sign"],true);
			//写日志记录
			//if ($isSign) {
			//	$isSignStr = 'true';
			//}
