...
class tab_sys_area {
	...
	static function on_save($arr_fields , $where = '') {
		...
		if( empty($where) ) {
			...
			if($arr['code'] == 0) {
				...
				self::on_tongbu($arr_fields['area_pid'] , $arr_return['id']);
			} else {
			...
			//不允许修改父id，只能通过移动
			if(isset($arr_fields['area_pid'])) unset($arr_fields['area_pid']);
			//修改数据表
	...
	static function on_tongbu($pid , $id = 0) {
		...
		if( !empty($pid) ) {
			$area_childs = 0;
			$obj_rs = $obj_db->get_one("select area_pids,area_childs from ".cls_config::DB_PRE."sys_area where area_id=".$pid);
			if(!empty($obj_rs) && !empty($obj_rs["area_pids"])) {
				$arr_update["area_pids"] = $obj_rs["area_pids"] . "," . $id;
				$arr_update["area_depth"] = count(explode("," , $arr_update["area_pids"]));
				$area_childs = $obj_rs['area_childs'];
			} else {
				$arr_update["area_pids"] = $id;
				$arr_update["area_depth"] = 1;
			}
			//计算父级子集数量
			$obj_rs = $obj_db->get_one("select count(1) as num from " . cls_config::DB_PRE . "sys_area where area_pid='" . $pid . "'");
			if(!empty($obj_rs) && $area_childs != $obj_rs['num']) {
				$obj_db->on_update(cls_config::DB_PRE."sys_area" , array('area_childs'=>$obj_rs['num']) , "area_id=" . $pid);
			}
		} else {
			$arr_update["area_pids"] = $id;
			$arr_update["area_depth"] = 1;
		}
		if(!empty($id)) $arr = $obj_db->on_update(cls_config::DB_PRE."sys_area" , $arr_update , "area_id=" . $id);
	}
	...
	static function on_delete($arr_id , $where = '') {
		...
		$arr_id = array();
		$obj_result = $obj_db->select( "select area_id,area_pid from " . cls_config::DB_PRE . "sys_area where " . $where );
		...
		if($arr_return['code']==0) {
			...
			foreach($arr_id as $item) {
				$arr = $obj_db->on_delete(cls_config::DB_PRE."sys_area" , $obj_db->concat(",",'area_pids',",") . " like '%," . $item['area_id'] . ",%'");
				if(!empty($item['area_pid'])) {
					self::on_tongbu($item['area_pid']);
				}
