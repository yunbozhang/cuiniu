	...
	static function on_save($arr_fields , $where = '') {
		...
		if( empty($where) ) {
			...
			if(!isset($arr_fields['group_sort']) || empty($arr_fields['group_sort'])) {
				$obj_rs = $obj_db->get_one("select max(group_sort) as sort from " . cls_config::DB_PRE . "meal_menu_group where group_pid=" . $arr_fields["group_pid"] . " and group_shop_id='" . $arr_fields['group_shop_id'] . "'");
