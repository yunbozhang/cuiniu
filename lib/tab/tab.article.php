...
class tab_article {
	...
	static function on_save($arr_fields,$where = '') {
		...
		if( empty($where) ) {
			...
			if(!isset($arr_fields['article_sort']) || empty($arr_fields['article_sort'])) {
				if(!isset($arr_fields["article_folder_id"])) $arr_fields["article_folder_id"] = 0;
	 ...
	 *
	 */
	static function on_copy($ids , $channel_id , $folder_id , $arr_where = array() , $about_id = '') {
		$arr_return = array("code"=>0,"msg"=>"");
		$obj_db = cls_obj::db_w();
		...
		while($obj_rs = $obj_db->fetch_array($obj_result))
		{
			...
			$arr_fields["article_about_id"]= (empty($about_id)) ? $obj_rs['article_about_id'] : $about_id;
	 ...
	 *
	 */
	static function on_cut($ids , $channel_id , $folder_id , $about_id = '') {
		$obj_db = cls_obj::db_w();
		$arr_update = array("article_folder_id" => $folder_id , "article_channel_id" => $channel_id) ;
		if(!empty($about_id)) $arr_update["article_about_id"] = $about_id;
		$arr_return = cls_obj::db_w()->on_update(cls_config::DB_PRE."article" , $arr_update , "article_id in(" . $ids . ")");
		return $arr_return;
	}
